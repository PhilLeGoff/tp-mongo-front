...
export default function SearchBar() {
  ...
  const { data: movies, isLoading } = useQuery({
    queryKey: ['search', debouncedQuery, selectedGenre],
    queryFn: async () => {
      const res = await axios.get('http://localhost:5000/films/search', {
        params: { q: debouncedQuery, genre: selectedGenre }
      });
      return res.data;
    },
    enabled: !!debouncedQuery || !!selectedGenre,
  });
  ...
  return (
      ...
      >
        ...
        {/*        Your Email*/}
        {/*    </label>*/}
        {/*    <button id="dropdown-button" data-dropdown-toggle="dropdown" className="shrink-0 z-10 inline-flex items-center py-2.5*/}
        {/*    px-4 text-sm font-medium text-center*/}
        ...
        {/*        <li>*/}
        {/*            <button type="button" className="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600*/}
        ...
        {/*        <input type="search" id="search-dropdown" className="block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50*/}
        {/*         rounded-e-lg border-s-gray-50 border-s-2 border border-gray-300 focus:ring-blue-500*/}
        ...
        {/*        <button type="submit" className="absolute top-0 end-0 p-2.5 text-sm font-medium h-full text-white bg-blue-700*/}
       ...
       {/* Search Results */}
          {isLoading && <p className="text-center mt-2">Searching...</p>}
          {movies && (
            <ul className="mt-4 space-y-2">
              {movies.map((movie) => (
                <li key={movie._id} className="bg-gray-100 dark:bg-gray-800 p-2 rounded">
                  {movie.title}
                </li>
              ))}
            </ul>
